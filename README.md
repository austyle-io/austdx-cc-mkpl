...
Add the marketplace and install the plugins. Pick whichever surface you
prefer — both work identically.

### From the terminal

```bash
claude plugin marketplace add austyle-io/austdx-cc-mkpl
claude plugin install modern-unix-tools@austdx-cc-mkpl
...
To browse and enable/disable plugins interactively, type `/plugin` from
inside any session.

For the rest of the command surface — update, uninstall, enable/disable,
prune, validate, tag, and the marketplace lifecycle — see
[AGENTS.md → Claude Code plugin & marketplace command reference](./AGENTS.md#claude-code-plugin--marketplace-command-reference).
